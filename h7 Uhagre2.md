...
## 1.1 Terminology:
- Viesti (plaintext) muutetaan salaukseksi (ciphertext) ja puretaan takaisin avaimen avulla. Kryptografia suojaa viestejä, kun taas kryptanalyysi yrittää murtaa ne. Tärkeintä on, että salaus perustuu avaimeen, ei itse algoritmin salaisuuteen – ja mielestäni tämä on looginen ja turvallinen periaate, koska avoimet ja testatut menetelmät ovat luotettavampia. Salaus takaa myös tunnistamisen, eheyden ja kiistämättömyyden. On olemassa symmetrinen salaus (sama avain molempiin suuntiin) ja epäsymmetrinen salaus (eri avaimet). Lopuksi erotetaan koodit (sanojen korvaus) ja salausalgoritmit, jotka toimivat bittitasolla. Tämä luku tekee selväksi, että tietoturvan ydin on avoimuus ja vahva matematiikka, ei salailu.

## 1.4 Simple XOR: 
- XOR (exclusive-or, C: ^) on bittien perusoperaatio, jossa esim. a^a=0 ja a^b^b=a. Yksinkertainen XOR-salaus, jossa teksti XORataan toistuvalla avaimella, on symmetrinen mutta erittäin heikko se vastaa käytännössä vanhaa Vigenèren sifferiä. Avaimen pituus ja viestin sisältö on helppo selvittää laskemalla yhtymäpisteitä, ja murtaminen kestää tietokoneella vain sekunteja. Monet ohjelmat väittävät tämän olevan “DES-tason” suojaus, mutta todellisuudessa se on vain lelusalaus. Mielestäni tällaiset menetelmät antavat vain väärän turvallisuuden tunteen oikeasti tarvitaan avoimia ja testattuja salausalgoritmeja.

## 1.7 Large Numbers: 
- Luvussa käsitellään suuria lukuja, joita käytetään kryptografiassa kuvaamaan esimerkiksi todennäköisyyksiä tai aikaskaaloja. Koska tällaiset luvut voivat helposti menettää merkityksensä, taulukossa 1.1 esitetään niille konkreettisia vertauksia, kuten todennäköisyys voittaa lotossa tai joutua salaman iskemäksi. Nämä luvut vaihtelevat arkisista todennäköisyyksistä aina kosmisiin aikakausiin, kuten Auringon elinkaareen tai maailmankaikkeuden ikään. Mielestäni tämä vertailu havainnollistaa hyvin, kuinka valtavia ja käsittämättömiä jotkin kryptografiassa käytetyt numerot todella ovat, ne ylittävät täysin kaiken, mihin ihminen pystyy arjessa samaistumaan.
...
## Lähteet
<a href="https://learning.oreilly.com/library/view/applied-cryptography-protocols/9781119096726/08_chap01.html#chap01-sec001">E-Kirja</a>, <a href="https://stackoverflow.com/">Stack Overflow</a>,<a href="https://cryptopals.com/sets/1">Cryptopals</a>
